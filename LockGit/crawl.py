...
URL = "https://www.twitter.com"  # 要爬取的网站
...
while (len(url_queue)):
    ...
    for image in images - url_image:
        try:
            ...
            file_path = "%s/" % (CRAWL_IMAGES_DIR,) + "%02d-%02d-%02d-" % (
                year, month, day) + "%02d:%02d:%02d-" % (hour, minute, second) + filename
    ...
    # 获取网页上所有url
    links = {urllib.parse.urljoin(response.url, url) for url in content_body.xpath(
        '//a/@href') if urllib.parse.urljoin(response.url, url).startswith('http')}
